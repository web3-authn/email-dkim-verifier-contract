...
This contract uses a short‑lived `request_id` (embedded in the email Subject) so a frontend can poll the DKIM verification result on‑chain without talking to the relayer.
...
- Request‑ID format (preferred for email recovery UX):
...
- Write paths:
  ...
  - `on_email_verification_private_result`:
    ...
    - If Outlayer fails before the worker can decrypt/parse the Subject, callers should pass a `request_id` hint into the encrypted verification call so the contract can still store a terminal failure result for frontend polling.
...
- Verified by integration test: `email-dkim-verifier-contract/tests/request_id_clearing.rs`.
