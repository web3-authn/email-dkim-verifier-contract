...
export function Step1RegisterOrLogin({
    ...
    switch (event.phase) {
      ...
      case RegistrationPhase.STEP_2_KEY_GENERATION:
        if (event.status === RegistrationStatus.SUCCESS) {
          toast.success(event.message || "Keys generated...", { id: toastId });
        } else {
  ...
  const handleSubmit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    if (loginState.isLoggedIn) {
      log.setOutputText("error", "Already logged in. Logout to register/login again.");
      return;
    }
    if (isLoading) return;

    const toastId = mode === "login" ? "login" : "registration";
